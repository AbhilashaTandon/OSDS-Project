...
/*
static void process_tablet_button_input(struct usb_gaomon *dev){

}

static void process_input(struct usb_gaomon *dev){
        unsigned char prev_byte = 0;
        unsigned char curr_byte = 0;
        for(; dev->input_processed < dev->input_copied; dev->input_processed++){
                curr_byte = *(dev->input_buffer + dev->input_processed);
                if(curr_byte != 0x08){
                ...
                switch(prev_byte){
                        case 0xe0:
                                process_tablet_button_input(dev);
                                break;
                        default:
                                break
...
retry:
        ...
        if (dev->input_filled) {
                ...
                size_t to_be_processed = dev->input_copied - dev->input_processed;
